...
遍历分布计算（解 $\boldsymbol{\pi} = \boldsymbol{\pi}\Pi$，即 $\Pi^\top$ 对应特征值1的左特征向量归一化）：

$$\pi_{\text{low}} \approx 0.355, \quad \pi_{\text{med}} \approx 0.251, \quad \pi_{\text{high}} \approx 0.395$$

验证：$\boldsymbol{\pi} \Pi = (0.355 \times 0.05 + 0.251 \times 0.607 + 0.395 \times 0.468, \ldots) = (0.355, 0.251, 0.395) = \boldsymbol{\pi}$ ✓

市场约39.5%的时间处于高波动状态，35.5%低波动，25.1%中波动。三个状态的时间占比相对均匀，高频切换（驻留仅1天）加高波动占主导，反映了后2016年地缘政治事件频发的市场环境。

**注意**：所有状态的自停留概率均恰好等于优化下界0.050，说明优化器被边界约束限制，真实的自停留概率可能更低（市场几乎每天都在切换regime）。
